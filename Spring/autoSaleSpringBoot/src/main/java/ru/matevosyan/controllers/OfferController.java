...
public class OfferController {
    ...
    private final OfferService<Offer> service;
     ...
     */
    @Autowired
    public OfferController(OfferService<Offer> service, FileUploader uploader,
    ...
    public List<JsonResponse> getAll() {
        return format.getResponseList(this.service.findAllOffers());
    }
...
//        offer.setUser(user);
     ...
     * @param offer model.
     * @return ResponseEntity of String type which contains true is request is OK else contains false.
     */
    @PutMapping(value = "/**/offerSellStatusValue")
    public ResponseEntity<String> change(@RequestBody Offer offer) {
        Integer changed = this.service.changeSellState(offer.getSoldState(), offer.getId());
        if (changed == 1) {
    ...
    public List<JsonResponse> lastDayOffers() {
        ...
        return format.getResponseList(this.service.findByPostingDate(start, end));
    }
    ...
    public List<JsonResponse> filterByBrand(HttpServletRequest req) {
        ...
        return format.getResponseList(this.service.findAllByCarBrand(brands));
    }
    ...
    public List<JsonResponse> getOffersWithPhoto() {
        ...
        return format.getResponseList(this.service.findByPicture(name));
    }
